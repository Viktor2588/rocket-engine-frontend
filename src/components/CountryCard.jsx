...
export default function CountryCard({ country, rank }) {
  ...
  const capabilities = [
    { key: 'humanSpaceflightCapable', label: 'Human Spaceflight', icon: '👨‍🚀' },
    { key: 'independentLaunchCapable', label: 'Independent Launch', icon: '🚀' },
    { key: 'reusableRocketCapable', label: 'Reusable Rockets', icon: '♻️' },
    { key: 'deepSpaceCapable', label: 'Deep Space', icon: '🌙' },
    { key: 'spaceStationCapable', label: 'Space Station', icon: '🛰️' },
  ];
  ...
  return (
    <Link to={`/countries/${country.isoCode}`}>
      <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition p-6 cursor-pointer border-l-4 border-indigo-500 relative">
        {/* Rank Badge */}
        {rank && (
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-xl font-bold text-gray-800 truncate">{country.name}</h3>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className={`p-3 rounded ${getScoreColor(score)}`}>
            <p className="text-xs text-gray-500">Capability Score</p>
            <p className="text-2xl font-bold">{score.toFixed(1)}</p>
          </div>
          <div className="bg-purple-50 p-3 rounded">
            <p className="text-xs text-gray-500">Region</p>
            <p className="text-sm font-semibold text-purple-700">{country.region}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4 text-center">
          ...
          <div className="bg-gray-50 p-2 rounded">
            <p className="text-xs text-gray-500">Astronauts</p>
            <p className="text-sm font-bold text-gray-700">
        ...
        <div className="flex flex-wrap gap-1 mb-4">
          ...
          {activeCapabilities.length === 0 && (
            <span className="text-xs text-gray-400">Emerging space program</span>
          )}
        </div>
        ...
        <div className="pt-4 border-t border-gray-200 flex justify-between items-center">
          ...
          <p className="text-xs text-indigo-500 font-semibold">View Details →</p>
        </div>
      </div>
    </Link>
  );
